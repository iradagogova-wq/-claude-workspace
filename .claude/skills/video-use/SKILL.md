...
6. **Never cut inside a word.** Snap every cut edge to a word boundary from the Deepgram transcript.
7. **Pad every cut edge.** Working window: 30–200ms. Deepgram timestamps drift 50–100ms — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached Deepgram transcript (Scribe-compatible schema)
...
- `DEEPGRAM_API_KEY` resolves — either in the environment or in `.env` at the video-use repo root. If missing, ask the user to paste one and write it to `.env` (never to the user's `<videos_dir>`).
...
- **`transcribe.py <video>`** — single-file Deepgram call. Cached. (`--num-speakers` accepted for compatibility but ignored — Deepgram diarization auto-detects speaker count.)
...
- **Audio events as signals.** `(laughs)`, `(sighs)`, `(applause)` mark beats. Extend past them. **Note:** Deepgram does not tag audio events the way ElevenLabs Scribe did — these markers won't appear in transcripts. Fall back to `timeline_view`'s waveform to spot laughs/reactions visually when it matters.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use hosted Deepgram.
