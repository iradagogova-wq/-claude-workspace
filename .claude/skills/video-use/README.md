...
Read install.md first to install this repo, wire up ffmpeg, register the skill with whichever agent you're running under, and set up the Deepgram API key — ask me to paste it when you need it. Then read SKILL.md for daily usage, and always read helpers/ because that's where the editing scripts live. After install, don't transcribe anything on your own — just tell me it's ready and wait for me to drop footage into a folder.
```

The agent handles the clone, dependencies, skill registration, and prompts you once for your Deepgram API key (grab one at [console.deepgram.com](https://console.deepgram.com/)).
...
**Layer 1 — Audio transcript (always loaded).** One Deepgram call per source gives word-level timestamps and speaker diarization. All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view. (Deepgram doesn't tag audio events like `(laughter)`/`(applause)` — use Layer 2 to spot those visually.)
